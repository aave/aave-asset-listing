...
describe('Deploy CRV assets with different params', () => {
  let whale: JsonRpcSigner;
  let crvHolder: JsonRpcSigner;
  ...
  before(async () => {
    ...
    await impersonateAccountsHardhat([AAVE_WHALE, CRV_HOLDER, AAVE_ORACLE_OWNER, DAI_HOLDER]);

    // impersonating holders
    whale = ethers.provider.getSigner(AAVE_WHALE);
    crvHolder = ethers.provider.getSigner(CRV_HOLDER);
    ...
    crv = (await ethers.getContractAt('IERC20', TOKEN, crvHolder)) as IERC20;
    ...
    await (await dai.transfer(CRV_HOLDER, parseEther('10'))).wait();
    await (
      await crv.transfer(
        proposer.address,
        (await crv.balanceOf(CRV_HOLDER)).sub(parseEther('10000'))
      )
    ).wait();
  ...
  it('Should list correctly an asset: borrow off, collateral off, stable rate off', async () => {
    ...
    await (await crv.connect(proposer).approve(pool.address, parseEther('2000'))).wait();
    ...
    await (await pool.deposit(crv.address, parseEther('100'), proposer.address, 0)).wait();
    ...
    await (await crv.connect(crvHolder).approve(pool.address, parseEther('200000'))).wait();
    await (await aave.connect(proposer).approve(pool.address, parseEther('200000'))).wait();

    // AAVE deposit by proposer
    await (await pool.deposit(aave.address, parseEther('100'), proposer.address, 0)).wait();

    // CRV deposit by crv holder
    await (
      await pool.connect(crvHolder).deposit(crv.address, parseEther('1000'), CRV_HOLDER, 0)
    ).wait();
    expect(await aCrv.balanceOf(CRV_HOLDER)).to.be.equal(parseEther('1000'));

    // CRV holder not able to borrow DAI against CRV
    await expect(
      pool.connect(crvHolder).borrow(dai.address, parseEther('1'), 2, 0, CRV_HOLDER)
    ).to.be.revertedWith(ERRORS.NO_COLLATERAL_BALANCE);

    // proposer not able to borrow CRV variable against AAVE
    await expect(
      pool.connect(proposer).borrow(crv.address, parseEther('10'), 2, 0, proposer.address)
    ).to.be.revertedWith(ERRORS.NO_BORROW);

    // proposer not able to borrow CRV stable against AAVE
    await expect(
      pool.borrow(crv.address, parseEther('5'), 1, 0, proposer.address)
    ).to.be.revertedWith(ERRORS.NO_BORROW);
  });
  it('Should list correctly an asset: borrow on, collateral off, stable borrow off', async () => {
    ...
    // proposer able to borrow CRV variable against AAVE
    await (
      await pool.connect(proposer).borrow(crv.address, parseEther('10'), 2, 0, proposer.address)
    ).wait();
    ...
    // proposer able to repay CRV variable
    await (await crv.connect(proposer).approve(pool.address, parseEther('100000'))).wait();
    await (await pool.repay(crv.address, MAX_UINT_AMOUNT, 2, proposer.address)).wait();
  ...
  it('Should list correctly an asset: borrow on, collateral on, stable rate off', async () => {
    ...
    const initCrvHolderBalance = await crv.balanceOf(CRV_HOLDER);
    await (await crv.connect(crvHolder).approve(pool.address, parseEther('200000'))).wait();
    await (await aave.connect(proposer).approve(pool.address, parseEther('200000'))).wait();
    ...
    // CRV holder able to borrow DAI against CRV
    await (
      await pool.connect(crvHolder).borrow(dai.address, parseEther('1'), 2, 0, CRV_HOLDER)
    ).wait();
    ...
    // crv holder able to repay DAI with  interests
    await (await dai.connect(crvHolder).approve(pool.address, MAX_UINT_AMOUNT)).wait();
    await (await pool.connect(crvHolder).repay(dai.address, MAX_UINT_AMOUNT, 2, CRV_HOLDER)).wait();

    // crv holder able to withdraw crv with interest
    await (await pool.connect(crvHolder).withdraw(crv.address, MAX_UINT_AMOUNT, CRV_HOLDER)).wait();
    expect(await crv.balanceOf(CRV_HOLDER)).to.be.gt(initCrvHolderBalance);
  });
  it('Should list correctly an asset: borrow on, collateral on, stable rate on', async () => {
    ...
    // fetching and testing pool config data for crv
    const {
      configuration: { data },
      aTokenAddress,
      stableDebtTokenAddress,
    ...
    // proposer able to borrow CRV stable against AAVE
    await (await pool.borrow(crv.address, parseEther('5'), 1, 0, proposer.address)).wait();
    ...
    // proposer able to repay CRV sttable
    await (await pool.repay(crv.address, MAX_UINT_AMOUNT, 1, proposer.address)).wait();
    expect(await variableDebt.balanceOf(proposer.address)).to.be.equal(parseEther('0'));
