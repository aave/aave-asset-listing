...
config({ path: path.resolve(process.cwd(), '.pax.env') });
...
const PAX_HOLDER = '0x7e62b7e279dfc78deb656e34d6a435cc08a44666';
const AAVE_ORACLE = '0x3a08ebbab125224b7b6474384ee39fbb247d2200';
...
describe('Deploy PAX assets with different params', () => {
  let whale: JsonRpcSigner;
  let paxHolder: JsonRpcSigner;
  ...
  before(async () => {
    ...
    await (
      await selfDestructContract.destroyAndTransfer(PAX_HOLDER, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
    await impersonateAccountsHardhat([AAVE_WHALE, PAX_HOLDER, DAI_HOLDER]);

    // impersonating holders

    whale = ethers.provider.getSigner(AAVE_WHALE);
    paxHolder = ethers.provider.getSigner(PAX_HOLDER);
    ...
    pax = (await ethers.getContractAt('IERC20', TOKEN, paxHolder)) as IERC20;
    oracle = (await ethers.getContractAt('IAaveOracle', AAVE_ORACLE)) as IAaveOracle
    decimalMultiplier = BigNumber.from('10').pow(await pax.decimals());
    ...
    // giving just a bit of Dai to PAX holder to pay for interest later
    await (await dai.transfer(PAX_HOLDER, parseEther('10'))).wait();
    await (
      await pax.transfer(
        proposer.address,
        (await pax.balanceOf(PAX_HOLDER)).sub((parseEther('1000').div(decimalMultiplier)))
      )
    ).wait();
    ...
    await rawBRE.run('create:proposal-new-asset:pax');
  ...
  it('Should list correctly an asset: borrow on, collateral off, stable borrow off', async () => {
    ...
    // preparing for tests.
    aPax = (await ethers.getContractAt('IERC20', aTokenAddress, proposer)) as IERC20;
    ...
    await (await pax.connect(paxHolder).approve(pool.address, parseEther('200000'))).wait();
    ...
    // PAX deposit by ampl holder
    const depositedAmount = parseEther('100').div(decimalMultiplier);
    await (
      await pool.connect(paxHolder).deposit(pax.address, depositedAmount, PAX_HOLDER, 0)
    ).wait();
    expect(await aPax.balanceOf(PAX_HOLDER)).to.gte(depositedAmount.sub(1));
    expect(await aPax.balanceOf(PAX_HOLDER)).to.lte(depositedAmount.add(1));

    // PAX holder not able to borrow DAI against PAX
    await expect(
      pool.connect(paxHolder).borrow(dai.address, parseEther('1'), 2, 0, PAX_HOLDER)
    ).to.be.revertedWith(ERRORS.NO_COLLATERAL_BALANCE);

    // proposer able to borrow PAX variable against AAVE
    const borrowAmount = parseEther('10').div(decimalMultiplier);
    await (
      await pool.connect(proposer).borrow(pax.address, borrowAmount, 2, 0, proposer.address)
    ).wait();
    expect(await variableDebt.balanceOf(proposer.address)).to.be.equal(borrowAmount);

    // proposer not able to borrow PAX stable against AAVE
    await expect(
      pool.borrow(pax.address, borrowAmount, 1, 0, proposer.address)
    ).to.be.revertedWith(ERRORS.NO_STABLE_BORROW);
    increaseTime(40000);

    // proposer able to repay PAX variable
    await (await pax.connect(proposer).approve(pool.address, parseEther('100000'))).wait();
    await (await pool.repay(pax.address, MAX_UINT_AMOUNT, 2, proposer.address)).wait();
    expect(await variableDebt.balanceOf(proposer.address)).to.be.equal(parseEther('0'));
  });

  it("Oracle should return a non zero PAX price", async () => {
